    ...
    const SETTINGS_KEY = 'ed_settings_v1';
    const CONTENT_KEY = 'ed_content_v1';
    const LEADERBOARD_KEY = 'ed_leaderboard_v1';
    const JOKE_STATS_KEY = 'ed_challenge_stats_v1';
    const NPR_ACHIEVEMENTS_KEY = 'ed_achievements_v1';
    const LEGACY_WORDS_KEY = 'ed_words_v1';
    const QUICK_GAME_KEY = 'ed_quick_game_v1';
    const USER_ID_KEY = 'ed_user_id_v1';
    const USER_ID_BACKUP_SCHEMA = 'emojidecoder.user-id.v1';
    const MULTI_DEVICE_GUEST_RESUME_KEY = 'ed_multidevice_guest_resume_v1';
    ...
    const LAST_TEAMS_KEY = 'ed_last_teams';
    const LAST_FFA_KEY = 'ed_last_ffa';
    const APP_STORAGE_PREFIX = 'ed_';
    ...
    const LEADERBOARD_MODE_KEYS = ['mimeTeams', 'mimeFfa', 'drawingTeams', 'drawingFfa', 'dailyMimeTeams', 'dailyMimeFfa'];
    ...
    const CATEGORY_KEYS = ['objetos', 'pessoas', 'filmes_series', 'lugares', 'marcas', 'expressoes'];
    ...
    const DEFAULT_WRONG_PENALTY_POINTS = 3;
    const DEFAULT_SPEED_BONUS_ENABLED = true;
    const DEFAULT_SPEED_BONUS_MAX = 5;
    const DEFAULT_HINT_PENALTY_POINTS = 3;
    const DIFFICULTY_BONUS_POINTS = { easy: 0, normal: 2, hard: 4 };
    ...
    const WORD_PACK_SCHEMA = 'emojidecoder.challengepack.v1';
    const PACK_SIGNATURE_ALGORITHM = 'ECDSA_P256_SHA256';
    const PACK_SIGNATURE_CONTEXT = 'emojidecoder-challenge-pack:v1';
    ...
    const APP_PUBLIC_URL = 'https://insight-x-lab-technologies.github.io/EmojiDecoder/';
    ...
    function getChallengeClue(entry) {
      if (typeof entry === 'string') return entry.trim();
      if (!entry || typeof entry !== 'object') return '';
      return String(entry.clue || entry.emoji || entry.text || entry.joke || entry.value || '').trim();
    }

    function getChallengeAnswer(entry) {
      if (!entry || typeof entry !== 'object') return '';
      return String(entry.answer || entry.solution || '').trim();
    }

    function getChallengeDifficulty(entry = {}) {
      const difficulty = String(entry?.difficulty || '').trim();
      return DIFFICULTY_KEYS.includes(difficulty) ? difficulty : 'easy';
    }
    ...
    function normalizeJokeEntry(entry, fallbackId = '') {
      ...
      return {
        ...
        text,
        clue: getChallengeClue(entry) || text,
        answer: getChallengeAnswer(entry),
        hint: String(entry?.hint || '').trim(),
        difficulty: getChallengeDifficulty(entry),
    ...
    function getMigratedJokeCategoryFromLegacy(category = '') {
      const mapping = {
        ...
        actions: 'expressoes',
        animals: 'objetos',
        movies: 'filmes_series',
        professions: 'pessoas',
        celebrities: 'pessoas'
      };
      return mapping[category] || 'objetos';
    }
      ...
      LEGACY_WORD_CATEGORY_KEYS.forEach(category => {
        ...
        DIFFICULTY_KEYS.forEach(diff => {
          ...
          words.forEach((word, index) => {
            ...
            if (joke) normalized[targetCategory].push({ ...joke, difficulty: diff });
          });
        });
      });
...
const READING_CHALLENGES_IT = [
  ...
  'Leggi come se avessi aspirato elio.'
    ...
    const EMOJI_DECODER_RULES_PT = [
      'Responda em até 3 palpites.',
      'Diga a categoria antes da resposta.',
      'Explique o caminho dos emojis antes de responder.',
      'O time adversário pode roubar se errar.',
      'Responda sem usar palavras em inglês.',
      'Peça uma única dica para o grupo antes de responder.',
      'A resposta precisa ser exata.',
      'Vale resposta aproximada se o grupo concordar.'
    ];

    const EMOJI_DECODER_RULES_EN = [
      'Answer in up to 3 guesses.',
      'Say the category before the answer.',
      'Explain the emoji path before answering.',
      'The opposing team may steal after a wrong answer.',
      'Answer without using Portuguese words.',
      'Ask the group for one hint before answering.',
      'The answer must be exact.',
      'Approximate answers count if the group agrees.'
    ];

    const getEmojiDecoderRules = locale => locale === 'en'
      ? EMOJI_DECODER_RULES_EN
      : EMOJI_DECODER_RULES_PT;
    ...
    function createCorePack() {
      return {
        id: CORE_PACK_ID,
        name: {
          pt: 'Pack Core de Desafios',
          en: 'Core Emoji Challenge Pack'
        },
        source: 'builtin',
        editable: true,
        enabled: true,
        ...
        readingChallenges: {
          pt: clone(getEmojiDecoderRules('pt')),
          en: clone(getEmojiDecoderRules('en')),
          es: clone(getEmojiDecoderRules('es')),
          fr: clone(getEmojiDecoderRules('fr')),
          de: clone(getEmojiDecoderRules('de')),
          it: clone(getEmojiDecoderRules('it'))
        }
      };
    }
    ...
    function buildPackIdFromName(name = '') {
      return `local-${buildJokeIdFromText(name || 'emoji-pack')}`.slice(0, 80);
    }

    function buildLocalPackContent(pack, options = {}) {
      const normalized = normalizePack(pack);
      const name = String(options.name || '').trim() || getPackDisplayName(normalized);
      const description = String(options.description || '').trim() || normalized.description || '';
      return {
        name: { [currentLanguage]: name },
        description: { [currentLanguage]: description },
        version: '1.0.0',
        author: 'EmojiDecoder',
        jokes: clone(normalized.jokes || {}),
        readingChallenges: clone(normalized.readingChallenges || {})
      };
    }

    function validateLocalPackExportEnvelope(envelope) {
      if (!envelope || typeof envelope !== 'object') throw new Error(t('packErrors.invalidJson'));
      if (envelope.schema !== WORD_PACK_SCHEMA) throw new Error(t('packErrors.invalidSchema'));
      if (envelope.export_type !== 'local') throw new Error(t('packErrors.invalidSchema'));
      if (!envelope.pack_id || envelope.pack_id === CORE_PACK_ID) throw new Error(t('packErrors.invalidPackId'));
      const pack = normalizePack({
        id: envelope.pack_id,
        name: envelope.content?.name || envelope.pack_id,
        description: envelope.content?.description || '',
        version: envelope.content?.version || '',
        author: envelope.content?.author || '',
        source: 'downloaded',
        editable: false,
        enabled: true,
        installedAt: new Date().toISOString(),
        jokes: envelope.content?.jokes || {},
        readingChallenges: envelope.content?.readingChallenges || envelope.content?.challenges || {}
      });
      if (getPackTotalContentCount(pack) === 0) throw new Error(t('packErrors.emptyPack'));
      return pack;
    }

    async function buildInstalledPackFromLocalEnvelope(envelope) {
      const pack = validateLocalPackExportEnvelope(envelope);
      if (envelope.content_sha256) {
        const contentHash = await sha256Base64Url(canonicalize(envelope.content || {}));
        if (contentHash !== envelope.content_sha256) throw new Error(t('packErrors.invalidContentHash'));
      }
      return pack;
    }
    ...
    function filterJokesByDifficulty(jokes, difficulty = gameState.difficulty) {
      const targetDifficulty = DIFFICULTY_KEYS.includes(difficulty) ? difficulty : 'easy';
      return normalizeJokeList(jokes).filter(joke => getChallengeDifficulty(joke) === targetDifficulty);
    }

    function countWordsForCategoryToken(category, diff = 'easy') {
      const premiumPack = getPremiumPackByToken(category);
      if (premiumPack) return filterJokesByDifficulty(getPremiumJokesForPack(premiumPack), diff).length;
      if (CATEGORY_KEYS.includes(category)) return filterJokesByDifficulty(getCoreJokesForCategory(category), diff).length;
      return 0;
    }
    ...
    function getDefaultSelectedCategories() {
      return ['objetos', 'filmes_series', 'expressoes'];
    }
    ...
    function isSpeedBonusEnabled() {
      const toggle = document.getElementById('speed-bonus-toggle');
      return toggle ? toggle.checked : DEFAULT_SPEED_BONUS_ENABLED;
    }

    function getConfiguredSpeedBonusMax() {
      return parsePointValue(document.getElementById('speed-bonus-input')?.value, DEFAULT_SPEED_BONUS_MAX);
    }

    function calculateSpeedBonus(timerLeft, timerTotal, bonusMax) {
      const safeTotal = Math.max(1, Number(timerTotal) || 1);
      const safeLeft = Math.max(0, Math.min(safeTotal, Number(timerLeft) || 0));
      const safeBonusMax = Math.max(0, Number(bonusMax) || 0);
      return Math.round((safeLeft / safeTotal) * safeBonusMax);
    }

    function getDifficultyBonusPoints(difficulty = gameState.currentWord?.difficulty || gameState.difficulty) {
      return DIFFICULTY_BONUS_POINTS[getChallengeDifficulty({ difficulty })] || 0;
    }

    function getCorrectScoreBreakdown() {
      const base = getConfiguredCorrectPoints();
      const speedBonus = isSpeedBonusEnabled()
        ? calculateSpeedBonus(gameState.timerLeft, gameState.timerDur, getConfiguredSpeedBonusMax())
        : 0;
      const hintPenalty = getCurrentHintPenaltyPoints();
      const difficultyBonus = getDifficultyBonusPoints();
      return {
        base,
        speedBonus,
        difficultyBonus,
        hintPenalty,
        total: Math.max(0, base + speedBonus + difficultyBonus - hintPenalty)
      };
    }
    ...
    function getComboStreak(playerName) {
      return Math.max(0, Number.parseInt(gameState.comboStreaks?.[playerName], 10) || 0);
    }

    function getComboMultiplier(playerName) {
      if (!gameState.comboMode || !playerName) return 1;
      return Math.min(3, getComboStreak(playerName) + 1);
    }

    function applyComboToScore(score, playerName) {
      const comboMultiplier = getComboMultiplier(playerName);
      return {
        ...score,
        comboMultiplier,
        total: Math.max(0, score.total * comboMultiplier)
      };
    }

    function recordComboSuccess(playerName) {
      if (!gameState.comboMode || !playerName) return;
      gameState.comboStreaks[playerName] = getComboStreak(playerName) + 1;
    }

    function resetComboStreak(playerName) {
      if (!gameState.comboMode || !playerName) return;
      gameState.comboStreaks[playerName] = 0;
    }
    ...
    let gameState = {
      ...
      stealMode: false,
      comboMode: false,
      typedGuessMode: false,
      dailyChallenge: false,
      dailyDate: '',
      comboStreaks: {},
      pendingBuzzScore: null,
      pendingStealScore: null,
      resultGuesser: '',
    ...
    let wbCat = 'objetos';
    let wbPreviewPackId = '';
    let leaderboardFilters = { mode: 'all', scope: 'standard' };
    ...
    function buildHostGamePayload() {
      ...
      const showSecretAnswerOnCompanion = shouldShowSecretAnswerOnCompanion();
      const answerText = String(gameState.currentWord?.answer || '').trim();
      const challengeText = String(gameState.currentWord?.word || '').trim();
      const helperText = getChallengeHelperText(gameState.currentWord, gameState.currentChallenge, {
        includeHint: gameState.hintRequested
      });
      const shouldRevealAnswer = showJokeOnCompanion && phase === 'score' && Boolean(answerText);
      const shouldShowSecretOnCompanion = showSecretAnswerOnCompanion && Boolean(answerText);
      const payload = {
        phase,
        gameType: gameState.gameType,
        buzzMode: Boolean(gameState.buzzMode),
        stealMode: Boolean(gameState.stealMode),
        comboMode: Boolean(gameState.comboMode),
        typedGuessMode: Boolean(gameState.typedGuessMode),
        ...
        deviceMode: showJokeOnCompanion || shouldShowSecretOnCompanion ? 'joke' : 'timer',
        statusKey: getGuestStatusKey(phase),
        currentPlayerName: currentPlayer.name,
        teamLabel: currentPlayer.teamLabel,
        jokeText: shouldShowSecretOnCompanion ? answerText : (showJokeOnCompanion ? challengeText : ''),
        challengeText: shouldShowSecretOnCompanion ? t('game.secretAnswerCompanionSub') : (showJokeOnCompanion ? helperText : ''),
        answerText: shouldRevealAnswer ? t('result.answerReveal', { answer: answerText }) : '',
        hintRequested: Boolean(gameState.hintRequested),
        difficulty: gameState.currentWord?.difficulty || gameState.difficulty,
        dailyChallenge: Boolean(gameState.dailyChallenge),
        dailyDate: gameState.dailyDate,
        secretMode: shouldShowSecretOnCompanion
      };
      return payload;
    }
    ...
    function attachHostConnection(conn) {
      ...
      conn.on('data', data => {
        ...
        if (data?.type === 'guest-guess') submitTypedGuess(data.guess);
      });
    ...
    function renderGuestSessionState(payload) {
      ...
      document.getElementById('guest-current-player-label').textContent = shouldShowJoke
        ? (payload.secretMode ? t('multiDevice.guestSecretAnswer') : (payload.answerText ? t('multiDevice.guestAnswerRevealed') : t('multiDevice.guestJokeTurn')))
      ...
      const answerEl = document.getElementById('guest-answer-display');
      const guestGuessPanel = document.getElementById('guest-guess-panel');
      timerCard?.classList.toggle('hidden', shouldShowJoke);
      jokeCard?.classList.toggle('hidden', !shouldShowJoke);
      if (shouldShowJoke) {
        const guestWord = document.getElementById('guest-word-display');
        const nextGuestWord = payload.jokeText || '---';
        const changed = guestWord && guestWord.textContent !== nextGuestWord;
        if (guestWord) guestWord.textContent = nextGuestWord;
        if (changed && payload.phase === 'playing') playDecodeAnimation(guestWord);
        syncChallengeA11y(guestWord, {
          clue: nextGuestWord,
          helper: payload.challengeText || '',
          answer: payload.answerText || '',
          secret: Boolean(payload.secretMode),
          announce: Boolean(changed)
        });
        ...
        if (payload.answerText) {
          if (answerEl) answerEl.textContent = payload.answerText;
          answerEl?.classList.remove('hidden');
        } else {
          answerEl?.classList.add('hidden');
        }
        guestGuessPanel?.classList.toggle('hidden', !payload.typedGuessMode || payload.secretMode || payload.phase !== 'playing');
      } else {
        challengeEl?.classList.add('hidden');
        answerEl?.classList.add('hidden');
        guestGuessPanel?.classList.add('hidden');
      }
      syncJokeTextSizing();
    }
    ...
    function exportUserId() {
      ...
      downloadJsonFile(`emojidecoder-user-id-${dateToken}.json`, createUserIdBackupEnvelope());
    ...
    function getPackPreviewWordEntries(pack, locale = currentLanguage) {
      ...
      CATEGORY_KEYS.forEach(category => {
        (bank[category] || []).forEach(joke => {
          ...
          if (text) {
            entries.push({
              word: getChallengeClue(joke) || text,
              answer: getChallengeAnswer(joke),
              hint: String(joke?.hint || '').trim(),
              category
            });
          }
        });
    ...
    function renderPackChallengePreviewItem(entry) {
      const wrapper = document.createElement('div');
      wrapper.className = 'pack-preview-challenge';

      const clue = document.createElement('div');
      clue.className = 'pack-preview-challenge-clue';
      clue.textContent = entry.word;
      wrapper.appendChild(clue);

      const metaParts = [
        getCategoryLabel(entry.category, { withIcon: true }),
        entry.answer ? t('wordbank.answerMeta', { answer: entry.answer }) : '',
        entry.hint ? t('wordbank.hintMeta', { hint: entry.hint }) : ''
      ].filter(Boolean);
      if (metaParts.length) {
        const meta = document.createElement('div');
        meta.className = 'pack-preview-challenge-meta';
    ...
    function renderPackPreview() {
      ...
      renderPreviewItems(
        ...
        renderPackChallengePreviewItem,
    ...
    function renderJokeRanking() {
      ...
      const curationFilter = document.getElementById('joke-curation-filter')?.value || 'topRated';
      const rankedJokes = Object.entries(loadJokeStats())
        ...
        .filter(Boolean)
        .filter(joke => {
          if (curationFilter === 'highSuccess') return joke.timesPlayed > 0 && joke.laughRate >= 0.7;
          if (curationFilter === 'needsReview') return (joke.ratingCount > 0 && joke.averageRating < 3) || (joke.timesPlayed >= 2 && joke.laughRate < 0.5);
          if (curationFilter === 'neverAgain') return joke.neverAgain;
          if (curationFilter === 'allStats') return joke.ratingCount > 0 || joke.timesPlayed > 0 || joke.neverAgain;
          return joke.ratingCount > 0;
        })
        .sort((a, b) => {
          if (curationFilter === 'highSuccess') {
            return b.laughRate - a.laughRate || b.timesPlayed - a.timesPlayed || a.word.localeCompare(b.word, currentLanguage);
          }
          if (curationFilter === 'needsReview') {
            return a.averageRating - b.averageRating || a.laughRate - b.laughRate || b.timesPlayed - a.timesPlayed || a.word.localeCompare(b.word, currentLanguage);
          }
          return b.averageRating - a.averageRating || b.laughs - a.laughs || b.timesPlayed - a.timesPlayed || a.word.localeCompare(b.word, currentLanguage);
        })
        .slice(0, 10);
      ...
      rankedJokes.forEach((joke, index) => {
        ...
        meta.textContent = [
          `⭐ ${formatJokeRatingValue(joke.averageRating)}`,
          `✅ ${formatJokeLaughRate(joke.laughRate)}%`,
          `${joke.timesPlayed}x`,
          joke.neverAgain ? '🚫' : ''
        ].filter(Boolean).join(' · ');
    ...
    function getDailyResultShareData() {
      ...
      return {
        title: t('share.title'),
        text: t('dynamic.dailyShareText', {
          date: gameState.dailyDate || getDailyChallengeDateKey(),
          winner: tie ? t('final.tie') : winner,
          score: entries || '-'
        }),
        url: getShareUrl()
      };
    ...
    async function shareDailyResult() {
      const shareData = getDailyResultShareData();
      if (canUseNativeShare(shareData)) {
        try {
          await navigator.share(shareData);
          return true;
          ...
          if (error?.name === 'AbortError') return false;
          console.warn('Daily result native sharing failed.', error);
        }
      }

      try {
        await copyTextToClipboard(`${shareData.text} ${shareData.url}`);
        showNotif(t('notifications.dailyResultCopied'));
        return true;
      } catch (error) {
        console.warn('Daily result could not be copied.', error);
        showNotif(t('notifications.shareCopyFailed'), 'var(--accent2)', 'var(--text)');
        return false;
      }
    }
    ...
    function collectSettings() {
      return {
        ...
        speedBonusEnabled: isSpeedBonusEnabled(),
        speedBonusMax: getConfiguredSpeedBonusMax(),
    ...
    function initializeSettings() {
      const defaults = {
        ...
        speedBonusEnabled: DEFAULT_SPEED_BONUS_ENABLED,
        speedBonusMax: DEFAULT_SPEED_BONUS_MAX,
        shuffleEnabled: true,
        theme: 'cosmic',
      ...
      document.getElementById('speed-bonus-toggle').checked = saved.speedBonusEnabled !== false;
      document.getElementById('speed-bonus-input').value = parsePointValue(saved.speedBonusMax, DEFAULT_SPEED_BONUS_MAX);
    ...
    function updateDiffWordCount() {
      ...
      document.getElementById('diff-word-count').textContent = t('dynamic.diffCount', {
        count: total,
        difficulty: t(`difficulty.${gameState.difficulty}`)
      });
    }
    ...
    function shouldShowSecretAnswerOnCompanion() {
      return hasActiveCompanionDevice()
        && Boolean(gameState.currentWord)
        && ['memorizing', 'playing', 'score'].includes(gameState.phase)
        && !isCurrentPlayerUsingCompanion();
    }
    ...
    function startGame() {
      ...
      gameState.gameType = 'mime';
      gameState.difficulty = document.getElementById('difficulty-select')?.value || 'easy';
      gameState.buzzMode = Boolean(document.getElementById('buzz-mode-toggle')?.checked);
      gameState.stealMode = Boolean(document.getElementById('steal-mode-toggle')?.checked);
      gameState.comboMode = Boolean(document.getElementById('combo-mode-toggle')?.checked);
      gameState.typedGuessMode = Boolean(document.getElementById('typed-guess-mode-toggle')?.checked);
      gameState.dailyChallenge = Boolean(document.getElementById('daily-challenge-toggle')?.checked);
      gameState.dailyDate = gameState.dailyChallenge ? getDailyChallengeDateKey() : '';
      gameState.comboStreaks = {};
      gameState.pendingBuzzScore = null;
      gameState.pendingStealScore = null;
      gameState.resultGuesser = '';
      refreshGameTypeUI();
    ...
    function playDecodeAnimation(element) {
      if (!element || !element.textContent || element.textContent.trim() === '---') return;
      const card = element.closest('.word-card');
      element.classList.remove('decoding-burst');
      card?.classList.remove('decoding-burst');
      void element.offsetWidth;
      element.classList.add('decoding-burst');
      card?.classList.add('decoding-burst');
    }

    function setA11yLiveText(text = '') {
      const live = document.getElementById('a11y-live-region');
      if (live) live.textContent = String(text || '').trim();
    }

    function buildChallengeA11yLabel({ clue = '', helper = '', answer = '', secret = false } = {}) {
      const label = secret ? t('game.secretAnswerLabel') : t('multiDevice.liveDrawing');
      return [label, clue, helper, answer ? t('result.answerReveal', { answer }) : '']
        .map(part => String(part || '').replace(/\s+/g, ' ').trim())
        .filter(Boolean)
        .join('. ');
    }

    function syncChallengeA11y(element, options = {}) {
      if (!element) return;
      const label = buildChallengeA11yLabel(options);
      if (!label) return;
      element.setAttribute('aria-label', label);
      if (options.announce) setA11yLiveText(label);
    }
    ...
    function syncHostJokePresentation() {
      ...
      if (!gameState.currentWord) {
        ...
        updateHintRequestControls();
        updateTypedGuessControls();
        syncJokeTextSizing();
        return;
      }

      if (gameWord) {
        const nextWord = gameState.currentWord.word;
        const changed = gameWord.textContent !== nextWord;
        gameWord.textContent = nextWord;
        if (changed && gameState.phase === 'playing') playDecodeAnimation(gameWord);
      }

      const helperText = getChallengeHelperText(gameState.currentWord, gameState.currentChallenge, {
        includeHint: gameState.hintRequested
      });
      const shouldShowChallenge = Boolean(helperText);
      syncChallengeA11y(gameWord, {
        clue: gameState.currentWord.word,
        helper: helperText,
        announce: gameState.phase === 'playing'
      });
      if (shouldShowChallenge) {
        if (gameChallengeTextEl) gameChallengeTextEl.textContent = helperText;
    ...
    function syncSecretAnswerPreview() {
      const card = document.getElementById('secret-answer-card');
      const text = document.getElementById('secret-answer-text');
      if (!card || !text) return;
      const answer = String(gameState.currentWord?.answer || '').trim();
      const shouldShowOnHost = Boolean(answer) && !shouldShowSecretAnswerOnCompanion();
      text.textContent = answer || '---';
      text.setAttribute('aria-label', buildChallengeA11yLabel({ answer, secret: true }));
      card.classList.toggle('hidden', !shouldShowOnHost);
    }

    function getChallengeHelperText(challenge, rule = '', options = {}) {
      const parts = [];
      const hint = String(challenge?.hint || '').trim();
      if (hint && options.includeHint) parts.push(t('game.hintPrefix', { hint }));
      if (rule) parts.push(t('game.rulePrefix', { rule }));
      return parts.join(' · ');
    }

    function normalizeGuessText(value = '') {
      const articles = new Set(['o', 'a', 'os', 'as', 'um', 'uma', 'uns', 'umas', 'the', 'an', 'el', 'la', 'los', 'las', 'lo', 'le', 'les', 'un', 'une', 'des', 'der', 'die', 'das', 'den', 'dem', 'ein', 'eine', 'il', 'gli']);
      return String(value)
        .toLocaleLowerCase(currentLanguage || DEFAULT_LANGUAGE)
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .replace(/[^a-z0-9]+/g, ' ')
        .trim()
    ...
    function isTypedGuessCorrect(guess, answer) {
      const normalizedGuess = normalizeGuessText(guess);
      const normalizedAnswer = normalizeGuessText(answer);
      return Boolean(normalizedGuess && normalizedAnswer && normalizedGuess === normalizedAnswer);
    }

    function updateTypedGuessControls() {
      const panel = document.getElementById('typed-guess-panel');
      const input = document.getElementById('typed-guess-input');
      const shouldShow = gameState.typedGuessMode && gameState.phase === 'playing' && Boolean(gameState.currentWord?.answer);
      panel?.classList.toggle('hidden', !shouldShow);
      if (!shouldShow && input) input.value = '';
    }

    function getCurrentHintText() {
      return String(gameState.currentWord?.hint || '').trim();
    }

    function getCurrentHintPenaltyPoints() {
      return gameState.hintRequested && getCurrentHintText() ? DEFAULT_HINT_PENALTY_POINTS : 0;
    }

    function updateHintRequestControls() {
      const panel = document.getElementById('hint-request-panel');
      const button = document.getElementById('request-hint-btn');
      const label = document.getElementById('hint-cost-label');
      if (!panel || !button || !label) return;

      const hasHint = Boolean(getCurrentHintText());
      const canRequest = gameState.phase === 'playing' && hasHint && !gameState.hintRequested;
      panel.classList.toggle('hidden', !hasHint || gameState.phase !== 'playing');
      button.disabled = !canRequest;
      button.textContent = gameState.hintRequested
        ? t('game.hintRequested')
        : t('game.requestHint', { points: DEFAULT_HINT_PENALTY_POINTS });
      label.textContent = t('game.hintCost', { points: DEFAULT_HINT_PENALTY_POINTS });
    }

    function requestHint() {
      if (gameState.phase !== 'playing' || !getCurrentHintText() || gameState.hintRequested) return;
      gameState.hintRequested = true;
      syncHostJokePresentation();
      broadcastHostGameState();
    }

    function submitTypedGuess(rawGuess) {
      const guess = String(rawGuess || '').trim();
      if (!gameState.typedGuessMode || gameState.phase !== 'playing' || !guess) return;
      const answer = String(gameState.currentWord?.answer || '').trim();
      if (isTypedGuessCorrect(guess, answer)) {
        const input = document.getElementById('typed-guess-input');
        if (input) input.value = '';
        markResult(true);
        return;
      }
      showNotif(t('notifications.guessIncorrect'), 'var(--accent2)', 'var(--text)');
    }

    function submitHostTypedGuess() {
      submitTypedGuess(document.getElementById('typed-guess-input')?.value || '');
    }

    function submitGuestTypedGuess() {
      const input = document.getElementById('guest-guess-input');
      const guess = input?.value || '';
      if (!guess.trim() || !isGuestSessionOpen()) return;
      sendToGuest(multiDeviceState.hostConnection, { type: 'guest-guess', guess });
      if (input) input.value = '';
      showNotif(t('notifications.guessSent'));
    }
    ...
    function revealWord() {
      ...
      gameState.memInterval = setInterval(() => {
        ...
        if (gameState.memorizeLeft > 0) {
          ...
          updateScoreManagerButton();
          syncHostJokePresentation();
          playDecodeAnimation(document.getElementById('word-display'));
          broadcastHostGameState();
          playAlertBeep(880);
          startTimer();
        }
    ...
    // ============================================================
    function getDailyChallengeDateKey(date = new Date()) {
      return date.toISOString().slice(0, 10);
    }

    function hashDailyToken(token = '') {
      let hash = 2166136261;
    ...
    function selectDailyChallenge(candidates = []) {
      if (!candidates.length) return null;
      const categoryToken = normalizeSelectedCategories(gameState.selectedCategories).join(',');
      const seed = [
        gameState.dailyDate || getDailyChallengeDateKey(),
        currentLanguage,
        gameState.difficulty,
        categoryToken,
        gameState.turnsDone
      ].join('|');
      return candidates[hashDailyToken(seed) % candidates.length];
    }
    ...
    function pickWord() {
      ...
      gameState.selectedCategories.forEach(category => {
        ...
        filterJokesByDifficulty(jokes, gameState.difficulty).forEach(joke => {
          const text = getJokeText(joke);
          if (!text) return;
          allWords.push({
            id: joke.id,
            ...
            answer: getChallengeAnswer(joke),
            hint: String(joke?.hint || '').trim(),
            difficulty: getChallengeDifficulty(joke),
            cat: category,
            category,
      ...
      if (available.length === 0) return { word: '???', cat: 'objetos' };

      const picked = gameState.dailyChallenge
        ? selectDailyChallenge(available)
        : (shuffle
    ...
    function playComboSound(multiplier = 1) {
      if (!isNavigationSoundEnabled() || multiplier <= 1) return;
      try {
        const ctx = new (window.AudioContext || window.webkitAudioContext)();
        const gain = ctx.createGain();
        gain.connect(ctx.destination);
        ...
        [659, 880, 1175].slice(0, Math.min(3, multiplier)).forEach((freq, index) => {
          const osc = ctx.createOscillator();
          osc.type = 'triangle';
          osc.connect(gain);
    ...
    function resetResultGuesserPicker() {
      const picker = document.getElementById('result-guesser-picker');
      const select = document.getElementById('result-guesser-select');
      const stealPanel = document.getElementById('result-steal-panel');
      const stealButton = document.getElementById('result-steal-btn');
      const nextButton = document.getElementById('result-next-turn-btn');
      picker?.classList.add('hidden');
      stealPanel?.classList.add('hidden');
      if (select) {
        select.innerHTML = `<option value="">${t('result.guesserPlaceholder')}</option>`;
        select.value = '';
        select.disabled = false;
      }
      if (stealButton) stealButton.disabled = false;
    ...
    function getPlayerName(player) {
      return String(player?.name || player || '').trim();
    }

    function getBuzzScoringPlayer(playerName) {
      return gameState.players.find(player => getPlayerName(player) === playerName) || null;
    }

    function getBuzzScoringSummary(playerName, points) {
      const scoringPlayer = getBuzzScoringPlayer(playerName);
      if (gameState.mode === 'teams' && scoringPlayer?.team) {
        return t('dynamic.buzzCorrectTeamPoints', {
          playerName,
          teamName: gameState.teamNames[scoringPlayer.team] || getDefaultTeamName(scoringPlayer.team),
          points
        });
      }
      return t('dynamic.buzzCorrectPlayerPoints', { playerName, points });
    }

    function addBuzzScore(playerName, points) {
      const scoringPlayer = getBuzzScoringPlayer(playerName);
      if (!scoringPlayer) return false;
      if (gameState.mode === 'teams' && scoringPlayer.team) {
        addScore('team' + scoringPlayer.team, points);
      }
      addScore(playerName, points);
      return true;
    }

    function calculateStealPoints(points) {
      return Math.max(1, Math.round((Number(points) || 0) / 2));
    }

    function getNextStealPlayer() {
      if (!gameState.players.length) return null;
      const nextIndex = (gameState.currentPlayerIdx + 1) % gameState.players.length;
      return gameState.players[nextIndex] || null;
    }

    function getStealSummary(player, points) {
      const playerName = getPlayerName(player);
      if (gameState.mode === 'teams' && player?.team) {
        return t('dynamic.stealTeamPoints', {
          playerName,
          teamName: gameState.teamNames[player.team] || getDefaultTeamName(player.team),
          points
        });
      }
      return t('dynamic.stealPlayerPoints', { playerName, points });
    }

    function renderStealPanel(correctScore) {
      if (!gameState.stealMode || gameState.buzzMode) return;
      const stealPlayer = getNextStealPlayer();
      if (!stealPlayer) return;
      const stealPoints = calculateStealPoints(correctScore.total);
      const panel = document.getElementById('result-steal-panel');
      const summary = document.getElementById('result-steal-summary');
      const button = document.getElementById('result-steal-btn');
      if (!panel || !summary || !button) return;
      gameState.pendingStealScore = { playerName: getPlayerName(stealPlayer), points: stealPoints };
      summary.textContent = t('result.stealSummary', {
        playerName: gameState.pendingStealScore.playerName,
        points: stealPoints
      });
    ...
    function applySteal() {
      const pendingSteal = gameState.pendingStealScore;
      if (!pendingSteal?.playerName || !pendingSteal.points) return;
      const stealPlayer = getBuzzScoringPlayer(pendingSteal.playerName);
      if (!stealPlayer) return;
      if (gameState.mode === 'teams' && stealPlayer.team) {
        addScore('team' + stealPlayer.team, pendingSteal.points);
      }
      addScore(pendingSteal.playerName, pendingSteal.points);
      const sub = document.getElementById('resultSub');
      if (sub) sub.textContent = appendChallengeAnswerSummary(getStealSummary(stealPlayer, pendingSteal.points));
      const button = document.getElementById('result-steal-btn');
    ...
    function renderResultGuesserPicker(correctScore) {
      const picker = document.getElementById('result-guesser-picker');
      const select = document.getElementById('result-guesser-select');
      const nextButton = document.getElementById('result-next-turn-btn');
      if (!picker || !select || !nextButton) return;
      ...
      gameState.players.forEach(player => {
        ...
        option.textContent = gameState.mode === 'teams' && player.team
          ? `${playerName} · ${gameState.teamNames[player.team] || getDefaultTeamName(player.team)}`
          : playerName;
      ...
      gameState.pendingBuzzScore = correctScore;
      gameState.resultGuesser = '';
    }

    function selectResultGuesser(playerName) {
      const pendingScore = gameState.pendingBuzzScore;
      if (!pendingScore || !playerName) return;
      const comboScore = applyComboToScore(pendingScore, playerName);
      if (!addBuzzScore(playerName, comboScore.total)) return;
      recordComboSuccess(playerName);
      if (comboScore.comboMultiplier > 1) playComboSound(comboScore.comboMultiplier);
      gameState.resultGuesser = playerName;
      gameState.pendingBuzzScore = null;
      const select = document.getElementById('result-guesser-select');
      const nextButton = document.getElementById('result-next-turn-btn');
      if (select) {
      ...
      if (nextButton) nextButton.disabled = false;
      const sub = document.getElementById('resultSub');
      const scoreSummary = getBuzzScoringSummary(playerName, comboScore.total);
      if (sub) sub.textContent = appendChallengeAnswerSummary(`${scoreSummary} · ${t('dynamic.correctScoreBreakdown', comboScore)}`);
      renderScoreMini();
      launchConfetti();
      broadcastHostGameState();
    }
    ...
    function appendChallengeAnswerSummary(summaryText) {
      const answer = String(gameState.currentWord?.answer || '').trim();
      if (!answer) return summaryText;
      const answerText = t('result.answerReveal', { answer });
      return summaryText ? `${answerText} · ${summaryText}` : answerText;
    }
    ...
    function markResult(correct, timeUp = false) {
      ...
      const sub = document.getElementById('resultSub');
      const rawCorrectScore = getCorrectScoreBreakdown();
      const correctScore = gameState.buzzMode ? rawCorrectScore : applyComboToScore(rawCorrectScore, playerName);
      const correctPoints = correctScore.total;
      ...
      if (correct) {
        if (gameState.buzzMode) {
          renderResultGuesserPicker(rawCorrectScore);
        } else if (gameState.mode === 'teams') {
          addScore('team' + player.team, correctPoints);
          addScore(playerName, correctPoints);
        } else {
          addScore(playerName, correctPoints);
        }
        if (!gameState.buzzMode) recordComboSuccess(playerName);
        if (!gameState.buzzMode && correctScore.comboMultiplier > 1) playComboSound(correctScore.comboMultiplier);

        emoji.textContent = '🎉';
        title.textContent = t('result.correctTitle');
        title.style.color = 'var(--accent3)';
        const scoreSummary = gameState.mode === 'teams'
        ...
        sub.textContent = gameState.buzzMode
          ? appendChallengeAnswerSummary(t('result.buzzPickPrompt'))
          : appendChallengeAnswerSummary(`${scoreSummary} · ${t('dynamic.correctScoreBreakdown', correctScore)}`);
        if (!gameState.buzzMode) launchConfetti();
      } else {
        resetComboStreak(playerName);
        if (!gameState.buzzMode && wrongPenaltyPoints > 0) {
          if (gameState.mode === 'teams') {
          ...
          sub.textContent = appendChallengeAnswerSummary(timeUp
          ...
          sub.textContent = appendChallengeAnswerSummary(timeUp ? t('dynamic.timeUpNoPoints') : t('dynamic.skippedNoPoints'));
        }
        renderStealPanel(correctScore);
    ...
    function normalizeLeaderboardModel(model = {}) {
      const normalized = { version: 1, matches: 0, scopeMatches: { standard: 0, daily: 0 }, players: {} };
      ...
      normalized.scopeMatches.standard = Math.max(0, Number.parseInt(model?.scopeMatches?.standard, 10) || 0);
      normalized.scopeMatches.daily = Math.max(0, Number.parseInt(model?.scopeMatches?.daily, 10) || 0);
      return normalized;
    }
    ...
    function getLeaderboardModeKey(gameType = gameState.gameType, mode = gameState.mode) {
      if (gameState.dailyChallenge) return `dailyMime${mode === 'teams' ? 'Teams' : 'Ffa'}`;
    ...
    function recordLeaderboardFinalScore() {
      ...
      const scopeKey = gameState.dailyChallenge ? 'daily' : 'standard';
      const leaderboard = loadLeaderboard();
      const entries = collectFinalLeaderboardEntries();
      if (entries.length) {
        leaderboard.matches = (Number.parseInt(leaderboard.matches, 10) || 0) + 1;
        leaderboard.scopeMatches = leaderboard.scopeMatches || { standard: 0, daily: 0 };
        leaderboard.scopeMatches[scopeKey] = (Number.parseInt(leaderboard.scopeMatches[scopeKey], 10) || 0) + 1;
      }
      entries.forEach(entry => {
    ...
    function isDailyLeaderboardMode(modeKey) {
      return modeKey.startsWith('daily');
    }
    ...
    function getLeaderboardScopedModeKeys(filters = leaderboardFilters) {
      return LEADERBOARD_MODE_KEYS.filter(modeKey => {
        ...
        const scopeMatches = filters.scope === 'daily'
          ? isDailyLeaderboardMode(modeKey)
          : !isDailyLeaderboardMode(modeKey);
        return modeMatches && scopeMatches;
      });
    }
    ...
    function renderLeaderboard() {
      ...
      const totalMatches = Number.parseInt(leaderboard.scopeMatches?.[leaderboardFilters.scope], 10) || 0;
    ...
    function refreshFinalScreenCopy() {
      ...
      document.getElementById('share-daily-result-btn')?.classList.toggle('hidden', !gameState.dailyChallenge);
    ...
    function createLocalPackExportEnvelope() {
      const name = document.getElementById('pack-creator-name')?.value || '';
      const description = document.getElementById('pack-creator-description')?.value || '';
      const editablePack = getEditablePack();
      ensurePackLocale(editablePack, currentLanguage);
      const content = buildLocalPackContent(editablePack, { name, description });
      const packId = buildPackIdFromName(name || getPackDisplayName(editablePack));
      const envelope = {
      ...
      validateLocalPackExportEnvelope(envelope);
      return envelope;
    }

    async function createShareableLocalPackEnvelope() {
      const envelope = createLocalPackExportEnvelope();
      envelope.content_sha256 = await sha256Base64Url(canonicalize(envelope.content || {}));
      return envelope;
    }

    function createPackShareUrl(envelope) {
      const url = new URL(getShareUrl());
      url.hash = `pack=${encodeURIComponent(textToBase64Url(JSON.stringify(envelope)))}`;
      return url.href;
    }

    function renderPackShareQRCode(url) {
      const host = document.getElementById('pack-share-qr');
      if (!host) return;
    ...
    function exportLocalPack() {
      try {
        const envelope = createLocalPackExportEnvelope();
        const dateToken = new Date().toISOString().slice(0, 10);
        downloadJsonFile(`${envelope.pack_id}-${dateToken}.json`, envelope);
        showNotif(t('notifications.localPackExported'));
      } catch (error) {
    ...
    async function shareLocalPack() {
      try {
        const envelope = await createShareableLocalPackEnvelope();
        const url = createPackShareUrl(envelope);
        renderPackShareQRCode(url);
        await copyTextToClipboard(url);
        showNotif(t('notifications.localPackShareCopied'));
      } catch (error) {
        const message = error?.message || t('notifications.shareCopyFailed');
        showNotif(message, 'var(--accent2)', 'var(--text)');
      }
    }

    function getPackEnvelopeFromUrlHash() {
      const match = String(window.location.hash || '').match(/^#pack=([^&]+)/);
      if (!match) return null;
      return JSON.parse(base64UrlToText(decodeURIComponent(match[1])));
    }

    async function installSharedPackFromUrl() {
      const envelope = getPackEnvelopeFromUrlHash();
      if (!envelope) return;
      try {
        const pack = await buildInstalledPackFromLocalEnvelope(envelope);
        installBuiltPack(pack);
        window.history.replaceState(null, document.title, window.location.pathname + window.location.search);
      } catch (error) {
        showNotif(error?.message || t('packErrors.invalidJson'), 'var(--accent2)', 'var(--text)');
      }
    }

    function installBuiltPack(pack) {
    ...
    async function installWordPackFile(file) {
      ...
      const pack = envelope?.export_type === 'local'
        ? await buildInstalledPackFromLocalEnvelope(envelope)
        : await buildInstalledPackFromEnvelope(envelope);
      return installBuiltPack(pack);
    }
    ...
    function getWordEntriesForWordBank(locale = currentLanguage, category = wbCat) {
      ...
      (localizedBank[category] || []).forEach((joke, index) => {
        ...
        entries.push({
          packId: pack.id,
          word: getChallengeClue(joke) || word,
          answer: getChallengeAnswer(joke),
          hint: String(joke?.hint || '').trim(),
          index,
          category,
    ...
    function renderWordBank() {
      ...
      if (!entries.length) {
        ...
        empty.textContent = t('wordbank.emptyCategory');
      ...
      entries.forEach(entry => {
        ...
        card.append(text);
        if (entry.answer || entry.hint) {
          const meta = document.createElement('div');
          meta.className = 'joke-entry-meta';
          meta.textContent = [
            entry.answer ? t('wordbank.answerMeta', { answer: entry.answer }) : '',
            entry.hint ? t('wordbank.hintMeta', { hint: entry.hint }) : ''
          ].filter(Boolean).join(' · ');
    ...
    function renderChallengeBank() {
      ...
      if (!challenges.length) {
        const empty = document.createElement('div');
        empty.className = 'content-empty-state';
        empty.textContent = t('wordbank.challengesEmpty');
        cont.appendChild(empty);
      }
    ...
    function addWord() {
      ...
      const answerInput = document.getElementById('inp-new-answer');
      const hintInput = document.getElementById('inp-new-hint');
      const category = document.getElementById('inp-word-cat').value;
      const clue = inp.value.trim();
      const answer = answerInput?.value.trim() || '';
      const hint = hintInput?.value.trim() || '';
      if (!clue || !answer) {
        showNotif(t('notifications.challengeFieldsRequired'), 'var(--accent2)', 'var(--text)');
        return;
      }
      const currentBank = normalizeJokeBank(getCorePack().jokes?.[currentLanguage] || {});
      const alreadyExists = (currentBank[category] || []).some(joke => getChallengeClue(joke) === clue);
      if (alreadyExists) {
      ...
      editablePack.jokes[currentLanguage][category].push({
        id: buildJokeIdFromText(`${clue}-${answer}`),
        text: clue,
        clue,
        answer,
        hint
      ...
      showNotif(t('dynamic.wordAdded', { word: clue, category: categoryLabel, difficulty: categoryLabel }));
    }
    ...
    function handleAction(button) {
      ...
      if (action === 'share-daily-result') {
        animateButtonClick(button);
        return shareDailyResult();
      }
      ...
      if (action === 'submit-typed-guess') {
        animateButtonClick(button);
        return submitHostTypedGuess();
      }
      if (action === 'submit-guest-guess') {
        animateButtonClick(button);
        return submitGuestTypedGuess();
      }
      ...
      if (action === 'export-local-pack') return exportLocalPack();
      if (action === 'share-local-pack') return shareLocalPack();
    ...
    function registerEventListeners() {
      document.addEventListener('click', event => {
        ...
        if (leaderboardFilter) {
          ...
          if (filter === 'type' || filter === 'mode' || filter === 'scope') {
      ...
      ['correct-points-input', 'wrong-points-input', 'speed-bonus-input'].forEach(id => {
        document.getElementById(id)?.addEventListener('input', event => {
          const fallback = id === 'correct-points-input'
            ? DEFAULT_CORRECT_POINTS
            : (id === 'speed-bonus-input' ? DEFAULT_SPEED_BONUS_MAX : DEFAULT_WRONG_PENALTY_POINTS);
      ...
      document.getElementById('result-guesser-select')?.addEventListener('change', event => {
        selectResultGuesser(event.target.value);
      });
      ...
      document.getElementById('speed-bonus-toggle')?.addEventListener('change', saveSettings);
